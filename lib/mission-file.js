    ...
    */
    ...
    async processRepeats(parent) {
        ...
        for (let r = repeats.length - 1; r >= 0; r--) {
            ...
            let {replaceWith, hasNested:nested} = await this.processSingleRepeat(element, parent.__scope)
            hasNested |= nested
            ///////////////////////////////////////////////
    ...
    async processSingleRepeat(element, parentScope) {
        let hasNested = false
        let ret = {replaceWith: [], hasNested: false}
        let outerScope = { plugins: this.plugins }
        Object.assign(outerScope, this.data)
        ...
        for (let i = 0, l = range.length; i < l; i++) {
            ...
            for (let c = 0, cl = childElements.length; c < cl; c++) {
                ...
                if (childElements[c].name != "repeat") {
                    ...
                    let {replaceWith: rw, hasNested: hn} = await this.processSingleRepeat(childElements[c], scope)
                    replaceWith.push(...rw)
                   // hasNested |= hn
                }
            }
        }
        return {replaceWith, hasNested}
    }
    /**
    ...
    async processImportsElements(importEles) {
        for (let i = 0, l = importEles.length; i < l; i++) {
            ...
            if (ele.name === "import") {
                ...
                await this.processImportsElements(expanded.replaceWith)
