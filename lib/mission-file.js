    ...
    */
        ...
        for (let i = expands.length - 1; i >= 0; i--) {
            ...
            for (let c = 0, cl = childElements.length; c < cl; c++) {
                if (childElements[c].name === "repeat") {
                    ...
                    continue
                } else if (childElements[c].name === "expand") {
                    replaceWith.push(childElements[c])
                    count++
                    continue
                }
    ...
    async processAllExpands() {
        ...
        while (count) {
            ...
            let mission_data = this.findFirstElement(this.model, "mission_data")
            if (!mission_data) return
            count += await this.processExpandTemplates(mission_data)
            if (count) {
            ...
            for (let i = 0, l = range.length; i < l; i++) {
                ...
                for (let c = 0, cl = childElements.length; c < cl; c++) {
                    ...
                    if (childElements[c].name != "repeat") {
                        ...
                        // if(childElements[c].name == "expand") {
                        //     await this.shallowInterpolate(clone, scope)
                        // }
