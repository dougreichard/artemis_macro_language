...
const { interpolate, getDescendantProp } = require('./template-string')
// For jsdoc
require('./typedefs')
...
class Utils {
    ...
    lookup(s) {
        return getDescendantProp(this.data, s)
    }
    int(s) {
        return parseInt(s)
    }
    ...
    */
    ...
    async processRepeats(parent) {
        ...
        for (let r = repeats.length - 1; r >= 0; r--) {
            ...
            let replaceWith = await this.processSingleRepeat(element, parent.__scope)
            ///////////////////////////////////////////////
            // Replace the repeat with the expanded content
            parent.elements.splice(repeats[r].index, 1, ...replaceWith)
        }
    }
    async processSingleRepeat(element, parentScope) {
        let ret = []
        let outerScope = { plugins: this.plugins }
        Object.assign(outerScope, this.data)
        ...
        if (parentScope) {
            Object.assign(outerScope, parentScope)
        }
        if (element.__scope) {
        ...
        let range
        if (rangeName) {
            if (rangeName.includes("${") >= 0) {
                rangeName = await interpolate(element.attributes._range, outerScope)
            }
            try {
                let ctx = vm.createContext(outerScope)
                range = vm.runInContext(rangeName, ctx);
            } catch (err) {
                this.error({ message: `<repeat> _range sytax error: ${rangeName}\n${err.message}\n` })
                
                return ret
            }
            
            //range = getDescendantProp(outerScope, rangeName)
            if (rangeName.search(/[\{\(]/) >= 0) {
                rangeName = '_data'
            } 
            ...
            let _length = await interpolate(element.attributes._length, outerScope)
            range = { length: parseInt(_length), isIndex: true }
            if (range.length == 0) {
        ...
        if (element.attributes._as) {
            rangeAs = await interpolate(element.attributes._as, outerScope)
        } else if (rangeName) {
            rangeAs = rangeName.split('.').pop()
        }
        ...
        for (let i = 0, l = range.length; i < l; i++) {
            ...
            Object.assign(scope, outerScope)
            /// Don't added arrays they add number keys, template strings down' like
            if (!isActuallyArray) {
            ...
            if (rangeAs) {
                if (range.isIndex) {
                    scope[rangeAs] = i
                } else if (typeof (scope._item) === 'object') {
                    scope[rangeAs] = scope
                    Object.assign(scope, range[i])
                } else {
                    scope[rangeAs] = range[i]
                }
            }
            ...
            for (let c = 0, cl = childElements.length; c < cl; c++) {
                if (childElements[c].name != "repeat") {
                    ...
                    let rw = await this.processSingleRepeat(childElements[c], scope)
                    replaceWith.push(...rw)
                }
            }
        }
    ...
    async processImports() {
        ...
        await this.processExpandTemplates(imports)
        let importEles = this.findElements(imports)
        await this.processImportsElements(importEles)
    }
    async processImportsElements(importEles) {
        for (let i = 0, l = importEles.length; i < l; i++) {
            let ele = importEles[i]
            if (ele.name === "import") {
                await this.processImport(ele)
            } else if (ele.name === "repeat") {
                let expanded = await this.processSingleRepeat(ele, {})
                await this.processImportsElements(expanded)
            } else if (ele.name === "expand") {
            }
    ...
    async processImport(ele) {
        ...
        if (!as) {
            as = ele.attributes._as
        }
        let skip = ele.attributes.skip
        if (!skip) {
            skip = ele.attributes._skip
        }
        let outerScope = { plugins: this.plugins }
        Object.assign(outerScope, this.data)
        if (skip) {
            let checkSkip = await interpolate(skip, outerScope)
            if (checkSkip === 'true') return;
        }
        if (!name) {
            this.error({ message: `Import without a name` })
        } else {
            name = await interpolate(name, outerScope)
        }
