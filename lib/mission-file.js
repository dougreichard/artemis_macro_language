    ...
    processTemplateParameters(template, args) {
        ...
        for (let p = 0, l = params.length; p < l; p++) {
            ...
            if (args[name]) {
                ...
                this.error(`${template.attributes.name} missing parameter ${name}`)
                valid = false
            }
        }
        return valid
    ...
    */
    async processExpandTemplates(parent) {
        ...
        for (let i = expands.length - 1; i >= 0; i--) {
            ...
            let values = { plugins: this.plugins }
            Object.assign(values, ele.attributes)
            
            let childElements = template.elements
            let replaceWith = []
            for (let c = 0, cl = childElements.length; c < cl; c++) {
                if (childElements[c].name === "repeat") {
                    this.error(`Repeats are not allowed within templates`)
                    continue
                }
                let clone = await this.deepInterpolate(childElements[c], values)
                if (clone) {
    ...
    async processRepeats(parent) {
        ...
        for (let r = repeats.length - 1; r >= 0; r--) {
            ...
            for (let i = 0, l = range.length; i < l; i++) {
                ...
                if (skipFilter) {
                    let test = await interpolate(skipFilter, values)
                    if (test == 'true') {
                        continue
                    }
                }
